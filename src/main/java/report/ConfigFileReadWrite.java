...
public class ConfigFileReadWrite {
	...
	public synchronized static <T> Boolean write(String fileName, String key, T value) {
		...
		try {
			...
			try {
				...
				LOG.debug("File : " + fileName + " Was Updated For Added Property Key = " + key + " ,value = " + value);
				successStatus = true;
	...
	public synchronized static void writeToConfigFile(String TestcaseConfig)
	{
		FileWriter fw = null;
		...
		bw.write(TestcaseConfig);
			...
			try {
				bw.close();
				LOG.debug("BufferedWriter(bw) Instance Closed");
			} catch (IOException e) {
